...
//        ZXRouter.changeRootViewController(UINavigationController.init(rootViewController: webVC))
    ...
    func requestForLogin() ->Void{
        ...
        ZXNetwork.asyncRequest(withUrl: ZXAPI.api(address: ZXAPIConst.User.login), params: dic, sign: false, method: .post) { (success, status, content, string, errMsg) in
            ZXHUD.hide(for: self.view, animated: true)
            self.loginBtn.isEnabled = true
            if status == ZXAPI_SUCCESS {
        ...
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 1.0, execute:{
            ...
            if self.logonInvalid {
                ...
                if isNewUser == 1 {//新会员进入修改个人信息页面
                    ...
                    ZXRouter.changeRootViewController(UINavigationController.init(rootViewController: userInfoVC))
                }else if isNewUser == 0 {
                    ZXRouter.changeRootViewController(ZXRootController.zx_tabbarVC())
                }
            }
