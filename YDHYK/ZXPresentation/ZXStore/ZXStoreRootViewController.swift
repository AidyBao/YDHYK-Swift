...
/// Store BaseVC
class ZXSTUIViewController: ZXUIViewController {
    override var preferredCartButtonHidden: Bool { return false }
...
// 店铺首页
class ZXStoreRootViewController: ZXSTUIViewController {
    ...
    static func configVC(storeId: String, memberId:String, token:String) -> ZXStoreRootViewController {
    ...
    override func viewDidLoad() {
        ...
        self.tblHomeList.backgroundColor = UIColor.zx_subTintColor
        ...
        self.tblHomeList.zx_addHeaderRefresh(showGif: true, target: self, action: #selector(zx_refresh))
...
//    }
    ...
    fileprivate func loadStoreHomePage(showHUD: Bool) {
        ...
        ZXStoreHomeViewModel.getHomePage(storeId: ZXStoreParams.storeId) { (s, c, error, model) in
            ...
            if s {
                if let model = model {
                    ...
                    ZXEmptyView.show(in: self.tblHomeList, type: .storeEmpty, text: "线上药店准备中")
                }
            } else {
                ZXHUD.showFailure(in: self.view, text: error, delay: ZX.HUDDelay)
            }
        }
