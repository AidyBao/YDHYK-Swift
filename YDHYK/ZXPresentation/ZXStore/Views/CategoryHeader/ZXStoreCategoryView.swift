...
class ZXStoreCategoryView: UIView {
    ...
    convenience init (origin: CGPoint) {
        self.init(frame: CGRect(x: origin.x, y: origin.y, width: ZXBOUNDS_WIDTH, height: 180))
        self.backgroundColor = .clear
        
        contentView = UIView(frame: CGRect(x: 10, y: 0, width: ZXBOUNDS_WIDTH - 20, height: 180))
        contentView.backgroundColor = .white
        contentView.layer.cornerRadius = 5.0
        contentView.layer.shadowColor = UIColor.zx_tintColor.cgColor
        ...
        layout.itemSize = CGSize(width: floor((ZXBOUNDS_WIDTH - 20) / 4.0), height: 90)
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let model = self.storeModel,let sort = model.zx_sorts {
            ...
            if indexPath.row == count {//全部
                ...
                if let url = URL(string: ZXAPI.file(address: sort[indexPath.row].icon)) {
                    cell.imgvIcon.kf.setImage(with: url, placeholder : UIImage.Default.drug)
                } else {
