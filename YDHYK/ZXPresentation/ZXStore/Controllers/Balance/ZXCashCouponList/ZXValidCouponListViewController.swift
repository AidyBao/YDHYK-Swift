...
class ZXValidCouponListViewController: ZXSTUIViewController {
    ...
    func loadCouponList(showHUD: Bool) {
        if let storeIds = storeIds, let fullMoneys = fullMoneys {
            ...
            ZXDrugStoreViewModel.couponList(drugstoreIds: storeIds , fullMoneys: fullMoneys, completion: { (success, code, list, errorMsg) in
            ...
            ZXHUD.showFailure(in: self.view, text: "店铺ID列表为空", delay: ZX.HUDDelay)
        }
    }
}
