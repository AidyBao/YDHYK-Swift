...
protocol ZXDrugstoreListCellDelegate: NSObjectProtocol {
    func didDrugstoreListCellJoinMemberAction(_ sender:UIButton,_ model: ZXSearchModel)
    func didDrugstoreListCellLocationAction(_ sender:UIButton,_ model: ZXSearchModel)
    func didDrugstoreListCellTelAction(_ sender:UIButton,_ model: ZXSearchModel)
    func didDrugstoreListCellBuyAction(_ sender:UIButton,_ model: ZXSearchModel)
}
...
class ZXDrugstoreListCell: UITableViewCell {
    ...
    func loadData(_ mode: ZXSearchModel) {
        ...
        if mode.headPortraitStr.isEmpty == false {
            self.iconImg.kf.setImage(with: URL.init(string: mode.headPortraitStr), placeholder: UIImage.Default.empty, options: nil, progressBlock: nil, completionHandler: nil)
        }
