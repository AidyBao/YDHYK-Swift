...
extension ZXDrugCategoryViewController: UITableViewDelegate {
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if let c = self.categories?[topIndex],let child = c.childList,child.count > 0 {
            ...
            ZXEmptyView.hide(from: self.ccvSubCategory)
        } else {
             self.ccvSubCategory.contentInset = UIEdgeInsetsMake(0, -40, 0, 0)
            let emptyView = ZXEmptyView.show(in: self.ccvSubCategory, type: .noData, text: "无相关分类")
...
extension ZXDrugCategoryViewController: UICollectionViewDelegateFlowLayout {
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        return CGSize(width: ((ZXBOUNDS_WIDTH - 85) - 40) / 3.0, height: 90)
    }
