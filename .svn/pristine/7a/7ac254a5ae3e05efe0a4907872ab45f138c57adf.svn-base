...
extension ZXMyAddressViewController {
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if let model = self.dataArray.object(at: indexPath.section) as? ZXAddressModel {
            delegate?.zxAddressList(vc: self, selected: model)
            let vcList = self.navigationController?.viewControllers
            for (_,vc) in (vcList?.enumerated())! {
                if vc is ZXBalanceViewController {
