...
/// 会员信息
@objcMembers class ZXMemberInfoModel: NSObject {
    var memberId: String = ""
    var name: String = ""
...
/// 本地购物篮只存储药品 ID 和 数量
/// 跳转到药篮是，调取接口获取实时数据
@objcMembers class ZXDrugBuyModel: NSObject {
...
/// 购物篮信息
@objcMembers class ZXCartModel: NSObject {
    ...
    var zx_selectedJsonString: String? {
        if storeDrugList.count > 0 {
            ...
            return ZXUtils.jsonString(obj: arrID)
        }
...
class ZXCart: NSObject {
    private static var zxcart:ZXCartModel?
