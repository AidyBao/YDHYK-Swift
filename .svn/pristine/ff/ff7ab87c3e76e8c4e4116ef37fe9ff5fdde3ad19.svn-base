        ...
        get {
            if let model = self.balanceModel {
                ...
                if orderList.count > 0 {
                    return ZXUtils.jsonString(obj: orderList)
...
//                    }
    ...
    override func viewDidLoad() {
        ...
        self.lbTotalInfo.font = UIFont.zx_bodyFont(UIFont.zx_bodyFontSize)
    ...
    fileprivate func updateTotalPrice() {
        ...
        attrPrice.zx_appendFont(font: UIFont.zx_titleFont(UIFont.zx_subTitleFontSize), at: NSMakeRange(0, priceStr.count))
        attrPrice.zx_appendColor(color: UIColor.zx_customBColor, at: NSMakeRange(0, priceStr.count))
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if self.balanceModel == nil {
            ZXHUD.showFailure(in: self.view, text: "无相关订单数据", delay: ZX.HUDDelay)
        }
    }
    ...
    @IBAction func commitAction(_ sender: ZXRButton) {
        if let address = self.orderAddress {
            if let orderContent = self.orderContentJsonString {
                ...
                ZXDrugStoreViewModel.saveOrder(orderContent: orderContent, consignee: address.consignee, tel: address.tel, address: address.zx_address, completion: { (success, code, errorMsg) in
                    ...
                    if success {
                        ...
                        if code == ZXAPI_COUPON_ERROR {
                            ZXHUD.showFailure(in: self.view, text: "现金券异常,重新结算", delay: ZX.HUDDelay)
                ...
                ZXHUD.showFailure(in: self.view, text: "订单数据不存在", delay: ZX.HUDDelay)
            }
            
        } else {
            ZXHUD.showFailure(in: self.view, text: "请填写收货地址", delay: ZX.HUDDelay)
        }
    }
...
extension ZXBalanceViewController: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if let model = self.balanceModel {
            ...
            case 0://收货地址
                if let addressVC = ZXMyAddressViewController.zx_init() {
                    addressVC.delegate = self
                    self.navigationController?.pushViewController(addressVC, animated: true)
                }
            case 1 + storeGroupCount:
                if indexPath.row == 0 {//现金券
                    let cCount = model.couponCount
                    if cCount > 0 {
                        ...
                        ZXHUD.showFailure(in: self.view, text: "无可用的现金券", delay: ZX.HUDDelay)
                    }
                }
            default:
                ...
                if indexPath.row == 2 {//支付方式 配送方式
                    if index >= 0 {
                        ...
                        ZXHUD.showFailure(in: self.view, text: "信息不存在", delay: ZX.HUDDelay)
                    }
...
extension ZXBalanceViewController: ZXAddressListDelegate {
    ...
    func zxAddressList(vc: ZXMyAddressViewController, selected address: ZXAddressModel?) {
        if let address = address {
            ...
            sAddr.status = address.status
            sAddr.isDefault = address.isDefault
