...
class ZXCartButton: UIWindow {
    ...
    convenience init() {
        self.init(frame: CGRect(x: 10, y: ZXBOUNDS_HEIGHT - ZXCartButton.buttonWidth * 2, width: ZXCartButton.buttonWidth, height: ZXCartButton.buttonWidth))
        ...
        dotNum.backgroundColor = UIColor.zx_customBColor
        dotNum.font = UIFont.zx_titleFont(12)
    ...
    @objc func handlePanGesture(gesture: UIPanGestureRecognizer) {
        let window = ZXRootController.appWindow()
        ...
        case .ended:
            if let view = gesture.view {
                var stopPoint = CGPoint(x: 0, y: ZXBOUNDS_HEIGHT / 2.0)
                if (view.center.x < ZXBOUNDS_WIDTH / 2.0) {
                    if (view.center.y <= ZXBOUNDS_HEIGHT/2.0) {
                        //左上
                        ...
                        //左下
                        if (view.center.x  >= ZXBOUNDS_HEIGHT - view.center.y) {
                            stopPoint = CGPoint(x: view.center.x,y: ZXBOUNDS_HEIGHT - ZXCartButton.buttonWidth/2.0)
                        }else{
                            stopPoint = CGPoint(x: ZXCartButton.buttonWidth/2.0,y: view.center.y)
                        }
                    }
                    ...
                    if (view.center.y <= ZXBOUNDS_HEIGHT/2.0) {
                        //右上
                        if (ZXBOUNDS_WIDTH - view.center.x  >= view.center.y) {
                            stopPoint = CGPoint(x: view.center.x,y: ZXCartButton.buttonWidth/2.0)
                        }else{
                            stopPoint = CGPoint(x: ZXBOUNDS_WIDTH - ZXCartButton.buttonWidth/2.0,y: view.center.y)
                        }
                    }else{
                        //右下
                        if (ZXBOUNDS_WIDTH - view.center.x  >= ZXBOUNDS_HEIGHT - view.center.y) {
                            stopPoint = CGPoint(x: view.center.x,y: ZXBOUNDS_HEIGHT - ZXCartButton.buttonWidth/2.0)
                        }else{
                            stopPoint = CGPoint(x: ZXBOUNDS_WIDTH - ZXCartButton.buttonWidth/2.0,y:view.center.y)
                        }
                ...
                if (stopPoint.x + ZXCartButton.buttonWidth/2.0 >= ZXBOUNDS_WIDTH) {
                    stopPoint = CGPoint(x: ZXBOUNDS_WIDTH - ZXCartButton.buttonWidth/2.0,y: stopPoint.y)
                }
                ...
                if (stopPoint.y + ZXCartButton.buttonWidth/2.0 >= ZXBOUNDS_HEIGHT) {
                    stopPoint = CGPoint(x: stopPoint.x, y: ZXBOUNDS_HEIGHT - ZXCartButton.buttonWidth/2.0)
                }
