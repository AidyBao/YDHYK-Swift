        ...
        if itemList.count > 0 {
            ...
            for item in itemList {
                if let dicTT = item.mj_keyValues() {
                    dicTT["itemId"] = item.id
                    dicTT.removeObject(forKey: "id")
                    arrList.append(dicTT as! Dictionary<String, Any>)
                }
            }
            let tempString = (arrList as NSArray).mj_JSONString()
            //dicp["itemList"] = tempString
            dicp["itemList"] = tempString?.replacingOccurrences(of: "\\\"", with: "\\\\\"", options: .regularExpression, range: nil)
        }
