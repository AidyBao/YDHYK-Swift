    ...
    static var addressModelArray:NSMutableArray? {
        get {
            if addArray.count == 0 {
                ...
                if let cacheList = userDefaults.object(forKey: USERS_ADDRESS) as? Dictionary<String,Any> {
        ...
        set {
            if let newValue = newValue {
                ...
                userDefaults.set(dicStoreList, forKey: USERS_ADDRESS)
                userDefaults.synchronize()
            }else{
                let userDefaults = UserDefaults.standard
                if var cacheList = userDefaults.object(forKey: USERS_ADDRESS) as? Dictionary<String,Any> {
                    cacheList.removeValue(forKey: self.userId)
                    userDefaults.set(cacheList, forKey: USERS_ADDRESS)
                }else{
                    userDefaults.removeObject(forKey: USERS_ADDRESS)
                }
                userDefaults.synchronize()
            }
        }
