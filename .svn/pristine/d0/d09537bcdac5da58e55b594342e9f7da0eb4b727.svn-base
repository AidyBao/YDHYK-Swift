...
class ZXStoreHeaderView: UIView {
    ...
    */
    ...
    convenience init(origin: CGPoint) {
        self.init(frame: CGRect(x: origin.x, y: origin.y, width: ZXBOUNDS_WIDTH, height: 250))
        self.backgroundColor = UIColor.zx_subTintColor
        let view = UIView.init(frame: CGRect(x: 0, y: 0, width: ZXBOUNDS_WIDTH, height: 70))
        view.backgroundColor = UIColor.zx_tintColor
        self.addSubview(view)
        
        searchButton = UIButton.init(type: .custom)
        searchButton.frame = CGRect(x: 10, y: 10, width: ZXBOUNDS_WIDTH - 20, height: 40)
        searchButton.backgroundColor = .white
        searchButton.layer.cornerRadius = 5
        searchButton.setTitle("搜索药品名、适应症或功能主治", for: .normal)
        searchButton.titleLabel?.font = UIFont.zx_bodyFont(UIFont.zx_bodyFontSize - 1)
