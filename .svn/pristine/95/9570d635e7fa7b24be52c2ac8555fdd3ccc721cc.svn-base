...
class ZXUITextField: UITextField {
    ...
    override func draw(_ rect: CGRect) {
        ...
        // 键盘完成按钮
        let toolBar = UIToolbar(frame: CGRect.init(x: 0, y: 0, width: ZX_BOUNDS_WIDTH, height: 30))
        toolBar.barStyle = UIBarStyle.default
        
        let btnFished = UIButton(frame: CGRect.init(x: 0, y: 0, width: 50, height: 25))
        btnFished.setTitleColor(UIColor.zx_navBarColor, for: UIControlState.normal)
        btnFished.titleLabel?.font = UIFont.zx_bodyFont
        ...
        let space = UIView(frame:CGRect.init(x: 0, y: 0, width: ZX_BOUNDS_WIDTH - btnFished.frame.width - 30, height: 25))
