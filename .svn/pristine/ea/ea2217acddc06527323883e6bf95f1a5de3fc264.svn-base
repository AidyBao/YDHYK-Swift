...
class ZXEditProfileViewController: ZXUITableViewController {
    ...
    lazy var sexArray: NSMutableArray = {
        let arr: NSMutableArray = NSMutableArray.init(array: ["男","女"])
        return arr
    }()
...
extension ZXEditProfileViewController {
        ...
        ZXLoginManager.requestForGetAgeGroup { (succ, code, content, str, errMsg) in
            if code == ZXAPI_SUCCESS {
                if let data = content["data"],((data as? Array<Any>) != nil) {
                    let tempArr: NSMutableArray = ZXAgeGroupModel.mj_objectArray(withKeyValuesArray: data)
                    for (_, c) in tempArr.enumerated() {
                        let model = c as! ZXAgeGroupModel
                        self.ageArray.add(model.value)
                    }
                }
            }else{
    ...
    func requestForModiySex(_ sex: Int) {
        ZXHUD.showLoading(in: self.view, text: ZX_LOADING_TEXT, delay: nil)
        ZXEditProfileController.requestForModifySex(sex) { (suc, code, content, str, errMsg) in
            ZXHUD.hide(for: self.view, animated: true)
            if code == ZXAPI_SUCCESS {
                ZXHUD.showSuccess(in: self.view, text: content["msg"] as! String, delay: ZX.HUDDelay)
    ...
    func requestForModiyAgeGroup(_ ageGroup: Int, _ ageStr: String) {
        ZXHUD.showLoading(in: self.view, text: ZX_LOADING_TEXT, delay: nil)
        ZXEditProfileController.requestForModifyAgeGroup(ageGroup) { (suc, code, content, str, errMsg) in
            ZXHUD.hide(for: self.view, animated: true)
            if code == ZXAPI_SUCCESS {
                ZXHUD.showSuccess(in: self.view, text: content["msg"] as! String, delay: ZX.HUDDelay)
                ...
                ZXUser.user.ageGroups = ageGroup
                ZXUser.user.sync()
            }else{
                ZXHUD.showFailure(in: self.view, text: errMsg!, delay: ZX.HUDDelay)
            }
